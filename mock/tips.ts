...
import { searchTips as searchTipsUtil } from './utils/search';
...
export const searchTips = (query: string, allTips: Tip[]): Tip[] => {
  return searchTipsUtil(query, allTips, (tip) => [
      tip.topic,
      tip.level, 
      tip.answer || '',
...
export const mockPaginatedTips = (
  ...
  if (filters?.search) {
    filteredTips = searchTips(filters.search, filteredTips);
  }
