        ...
        for (const [sectionIndex, section] of exam.sections.entries()) {
            for (const [subsectionIndex, subsection] of section.subsections.entries()) {
                for (const [questionIndex, question] of subsection.questions.entries()) {
                    ...
                    // Gộp logic cho write_text và record_audio vì chúng có cùng yêu cầu validation
                    const isWriteOrRecordType = question.question_type_id === 'write_text' || question.question_type_id === 'record_audio';
                    ...
                    } else if (isWriteOrRecordType) {
                        // Câu hỏi điền từ/ghi âm có thể có hoặc không có đáp án (trường hợp tự luận)
