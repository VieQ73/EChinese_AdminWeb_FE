...
interface MCQTextQuestionFormProps {
...
export const MCQTextQuestionForm: React.FC<MCQTextQuestionFormProps> = ({
  question,
  templatePart,
  onSave,
  onCancel
  ...
  const handleSave = () => {
    ...
    const hasQuestionText = templatePart.allowed_fields.text ? !!formData.question_text.trim() : true;
    const hasOptionTexts = formData.options.every(opt => opt.text.trim());
      ...
      <div className="bg-white rounded-xl shadow-xl max-w-3xl w-full mx-4 max-h-[90vh] overflow-y-auto">
        ...
        <div className="p-6 space-y-6">
          ...
          {templatePart.allowed_fields.text && (
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-2">
                Nội dung câu hỏi {templatePart.input_type.includes('text') ? '*' : '(tuỳ chọn)'}
              </label>
              <textarea
                ...
                required={templatePart.input_type.includes('text')}
              />
            </div>
          )}
                ...
                <div key={option.id} className="flex items-center space-x-3">
                  <label className="flex items-center">
                    <input
                      ...
                      className="mr-2"
                    />
                    <span className="font-medium text-gray-700 w-8">{option.label}.</span>
                  </label>
                  ...
                  <input
                    ...
                    placeholder={`Nhập nội dung lựa chọn ${option.label}...`}
                    required
                  />
                </div>
              ))}
