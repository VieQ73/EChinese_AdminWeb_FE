...
import RemoveActionForm from './forms/RemoveActionForm';
import RestoreActionForm from './forms/RestoreActionForm';
...
interface ModerationActionModalProps {
    ...
    onConfirm: (data: Omit<FormData, 'ruleIds' | 'resolution' | 'severity'> & Partial<FormData>) => void;
}
...
const ModerationActionModal: React.FC<ModerationActionModalProps> = ({ isOpen, onClose, actionData, onConfirm }) => {
    ...
    // Nút xác nhận luôn bật nếu là self-action
    const isConfirmDisabled = isSelfAction
        ? false
        : (action === 'remove'
