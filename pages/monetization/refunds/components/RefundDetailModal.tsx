...
const RefundDetailModal: React.FC<RefundDetailModalProps> = ({ isOpen, onClose, refund, onProcess, isProcessing, currentUserRole }) => {
    if (!refund) return null;

    return (
        <Modal isOpen={isOpen} onClose={onClose} title={`Yêu cầu hoàn tiền #${refund.id}`} className="max-w-4xl">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                ...
                {/* Cột phải: Thông tin thanh toán gốc & Hành động */}
                <div className="space-y-6">
                    <div>
                        <h4 className="text-base font-semibold text-gray-800 mb-2">Thông tin giao dịch gốc</h4>
                    ...
                    {refund.status === 'pending' && (
                         <div>
                            <h4 className="text-base font-semibold text-gray-800 mb-2">Xử lý yêu cầu</h4>
                            <div className="p-4 rounded-lg border border-amber-300 bg-amber-50">
                                <ProcessRefundForm 
                                    refund={refund}
                                    onSubmit={onProcess}
                                    onCancel={() => {}} // Form tự quản lý cancel
