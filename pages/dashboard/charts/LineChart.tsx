...
const LineChart: React.FC<LineChartProps> = ({ data }) => {
    ...
    const { points, path, yAxisLabels } = useMemo(() => {
        if (!data || data.length === 0) return { points: [], path: '', yAxisLabels: [] };
        ...
        const yAxisLabels = Array.from({ length: yAxisLabelCount + 1 }).map((_, i) => {
            ...
            return {
                y: getY(value),
                label: i === 0 ? '0' : value >= 1000 ? `${(value / 1000).toFixed(0)}k` : value.toFixed(0),
            };
        });
    ...
    return (
        <div className="h-64 w-full" aria-label="Biểu đồ đường">
            <svg viewBox={`0 0 ${width} ${height}`} width="100%" height="100%">
                {/* Y-axis grid lines and labels */}
                {yAxisLabels.map((labelInfo, i) => (
                    <g key={i} className="text-gray-300">
                        <line x1={padding.left} y1={labelInfo.y} x2={width - padding.right} y2={labelInfo.y} stroke="currentColor" strokeWidth="0.5" />
                        <text x={padding.left - 8} y={labelInfo.y + 3} textAnchor="end" fontSize="10" fill="#6b7280">{labelInfo.label}</text>
                    </g>
                ))}

                {/* X-axis labels */}
                {points.map((point, i) => (
                    <text key={i} x={point.x} y={height - padding.bottom + 15} textAnchor="middle" fontSize="10" fill="#6b7280">{point.label}</text>
                ))}
