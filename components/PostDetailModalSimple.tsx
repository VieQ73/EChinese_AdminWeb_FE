...
import { X, Heart, Eye, MessageCircle, User, Calendar, Tag } from 'lucide-react';
import { Post } from '../types';

interface PostDetailModalSimpleProps {
  isOpen: boolean;
  onClose: () => void;
  post: Post | null;
}

const PostDetailModalSimple: React.FC<PostDetailModalSimpleProps> = ({ isOpen, onClose, post }) => {
  const [comments, setComments] = useState<any[]>([]);

  useEffect(() => {
    if (isOpen && post) {
      console.log('📝 [PostDetailModalSimple] Opened with post:', post);
    }
  }, [isOpen, post]);
          ...
          <div className="flex-1 overflow-y-auto p-6">
            ...
            <div className="space-y-6">
              ...
              <div className="flex items-center space-x-3 p-4 bg-gray-50 rounded-lg">
                ...
                <div>
                  <div className="font-semibold text-gray-900">
                    {(post as any).user_name || `User ${post.user_id}`}
                  </div>
              ...
              {(post as any).images && (post as any).images.length > 0 && (
                <div>
                  <h3 className="text-sm font-semibold text-gray-500 uppercase tracking-wider mb-2">
                    Hình ảnh ({(post as any).images.length})
                  </h3>
                  <div className="grid grid-cols-2 gap-4">
                    {(post as any).images.map((img: string, idx: number) => (
                      <img
                        key={idx}
                        src={img}
                        alt={`Image ${idx + 1}`}
                ...
                <div className="grid grid-cols-2 gap-4 text-sm">
                  ...
                  {(post as any).updated_at && (
                    <div className="flex justify-between p-3 bg-gray-50 rounded">
                      ...
                      <span className="text-gray-900">{formatDate((post as any).updated_at)}</span>
                    </div>
                  )}
                </div>
              </div>
...
export default PostDetailModalSimple;
