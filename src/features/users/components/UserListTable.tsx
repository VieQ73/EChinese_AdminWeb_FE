...
import { Eye, Edit, Lock, Unlock, Trash2, MoreVertical, Loader2 } from 'lucide-react'; // Icons
import {
    DropdownMenu,
    DropdownMenuContent,
    DropdownMenuItem,
...
import { deactivateUser, activateUser, deleteUser } from '../userApi'; // Import userApi
...
interface UserListTableProps {
  ...
  onRefresh: () => void; // Callback để làm mới danh sách sau khi thao tác
}

const UserListTable: React.FC<UserListTableProps> = ({ users, onViewUser, onEditUser, onRefresh }) => {
  const [loadingAction, setLoadingAction] = useState<string | null>(null); // Để theo dõi trạng thái loading của từng action
  ...
  const handleDeactivate = async (userId: string) => {
    if (window.confirm("Bạn có chắc chắn muốn khóa người dùng này không?")) {
      ...
      try {
        ...
        alert("Không thể khóa người dùng: " + (error as any).message);
      } finally {
  ...
  const handleActivate = async (userId: string) => {
    if (window.confirm("Bạn có chắc chắn muốn mở khóa người dùng này không?")) {
      ...
      try {
        ...
        alert("Không thể mở khóa người dùng: " + (error as any).message);
      } finally {
        setLoadingAction(null);
      }
    }
  ...
  const handleDelete = async (userId: string) => {
    if (window.confirm("Bạn có chắc chắn muốn XÓA VĨNH VIỄN người dùng này không? Hành động này không thể hoàn tác.")) {
      ...
      try {
        ...
        alert("Không thể xóa người dùng: " + (error as any).message);
      } finally {
        setLoadingAction(null);
      }
    }
                      ...
                      <div className="flex-shrink-0 h-10 w-10">
                        {user.avatar_url ? (
                          ...
                          <div className="h-10 w-10 rounded-full bg-teal-100 text-teal-700 flex items-center justify-center font-bold text-sm">
                            {user.name ? user.name[0] : (user.username ? user.username[0] : 'N/A')}
                          </div>
                        )}
                      </div>
                      <div className="ml-4">
